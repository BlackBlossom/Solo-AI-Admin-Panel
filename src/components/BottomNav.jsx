...
export default function BottomNav() {
  ...
  const renderNavItem = (item, isMobile = false, isLastItem = false) => {
    ...
    return (
      <div key={item.path} className={isMobile ? 'w-full' : 'flex-1'}>
        <Link
          to={item.path}
          onClick={() => setIsMenuOpen(false)}
          className={`relative flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl sm:rounded-2xl transition-all duration-300 group w-full ${
            isActive 
              ? 'text-white' 
        ...
        <div className="relative z-10 flex flex-col items-center gap-1">
          ...
          />
          <span className="text-[10px] sm:text-xs font-medium leading-tight whitespace-nowrap">{item.label}</span>
        </div>

        {/* Active indicator dot */}
        {isActive && (
          <div className="absolute -bottom-0.5 w-1.5 h-1.5 bg-white rounded-full shadow-lg" />
        )}
      </Link>
    ...
    <>
      ...
      >
        ...
        <div className="relative">
          ...
          {/* Main nav */}
          <div className="relative backdrop-blur-3xl bg-gradient-to-r from-[#190830]/80  to-[#190830]/80 border border-purple-500/30 rounded-3xl shadow-2xl px-3 py-3">
            <div className="flex items-center gap-2">
              ...
              {showMoreButton && (
                <div className="flex-1">
                  <button
                    ...
                    className={`relative flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl sm:rounded-2xl transition-all duration-300 w-full group ${
                  ...
                  >
                    <div className="relative z-10 flex flex-col items-center gap-1">
                      <Menu className={`w-5 h-5 flex-shrink-0 transition-all duration-300 ${
                        isMenuOpen || moreItems.some(item => item.path === location.pathname) ? 'scale-110' : 'group-hover:scale-105'
                      }`} />
                      <span className="text-[10px] sm:text-xs font-medium leading-tight whitespace-nowrap">More</span>
                    </div>
              ...
              {/* More Menu Dropdown */}
              <AnimatePresence>
                {isMenuOpen && showMoreButton && (
                  <motion.div
                    initial={{ opacity: 0, y: 10, scale: 0.95 }}
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    transition={{ duration: 0.2 }}
                    className="absolute bottom-full mb-4 right-3 min-w-[200px]"
                  >
                    {/* Glow for dropdown */}
              ...
              {showMoreButton && (
                <div className="flex-1">
                  ...
                  >
                    <div className="relative z-10 flex flex-col items-center gap-1">
                      {isMenuOpen ? (
                        <X className={`w-5 h-5 flex-shrink-0 transition-all duration-300 scale-110`} />
                      ) : (
                        <Menu className={`w-5 h-5 flex-shrink-0 transition-all duration-300 ${
                          moreItems.some(item => item.path === location.pathname) ? 'scale-110' : 'group-hover:scale-105'
                        }`} />
                      )}
                      <span className="text-[10px] sm:text-xs font-medium leading-tight whitespace-nowrap">More</span>
                    </div>
